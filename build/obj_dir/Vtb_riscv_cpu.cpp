...
void Vtb_riscv_cpu::eval_end_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+eval_end_step Vtb_riscv_cpu::eval_end_step\n"); );
#ifdef VM_TRACE
    // Tracing
...
std::unique_ptr<VerilatedTraceConfig> Vtb_riscv_cpu::traceConfig() const {
...
void Vtb_riscv_cpu___024root__trace_decl_types(VerilatedVcd* tracep);

void Vtb_riscv_cpu___024root__trace_init_top(Vtb_riscv_cpu___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    Vtb_riscv_cpu___024root__trace_decl_types(tracep);
    Vtb_riscv_cpu___024root__trace_init_top(vlSelf, tracep);
    tracep->popPrefix();
}

VL_ATTR_COLD void Vtb_riscv_cpu___024root__trace_register(Vtb_riscv_cpu___024root* vlSelf, VerilatedVcd* tracep);

VL_ATTR_COLD void Vtb_riscv_cpu::trace(VerilatedVcdC* tfp, int levels, int options) {
    if (tfp->isOpen()) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vtb_riscv_cpu::trace()' shall not be called after 'VerilatedVcdC::open()'.");
    }
    if (false && levels && options) {}  // Prevent unused
    tfp->spTrace()->addModel(this);
    tfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vtb_riscv_cpu___024root__trace_register(&(vlSymsp->TOP), tfp->spTrace());
}
